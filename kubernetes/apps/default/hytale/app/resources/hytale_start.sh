...
#
# --- ok8.sh patch ---
# Upstream run_server() executes $java_cmd unquoted, which only performs
# word-splitting and does not re-parse the embedded double quotes that
# build_java_command() wraps around paths (-jar "$SERVER_JAR_PATH", etc).
# Those literal quote characters end up in java's argv, so `java -jar
# "/path/…jar"` fails with "Unable to access jarfile "/path/…jar"".
# Fixed below by running the assembled string through eval so the shell
# re-parses the quoting correctly. Drop this file once upstream fixes it:
...
run_server() {
    ...
    if [ -n "$RUNTIME_CMD" ]; then
        ...
        ( tail -f "$AUTH_PIPE" 2>/dev/null || true ) | eval "$java_cmd" 2>&1 | stdbuf -oL -eL sed 's/\r$//' | stdbuf -oL -eL tee "$AUTH_OUTPUT_LOG"
    fi
}
